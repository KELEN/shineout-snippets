...
'expandTrigger': "类型: string\n默认值: 'click'\n说明: 节点展开触发方式，可选值为 \\['click', 'hover', 'hover-only']",
...
'loader': "类型: function(key, d)\n默认值: 无\n说明: 设置loader属性后，未定义children的节点视为动态加载节点，点击展开触发 loader事件，children 为 null 或者长度为 0 视为叶子节点",
'mode': "类型: \\[0|1|2|3\\]\n默认值: 无\n说明: 选中值模式，未设置值为单选<br />0: 只返回完全选中的节点，包含父节点<br />1: 返回全部选中的节点和半选中的父节点<br />2: 只返回选中的子节点<br />3: 如果父节点选中，只返回父节点",
'onChange': "类型: function(array)\n默认值: 无\n说明: 设置 onChange 属性时，显示 选择框。参数为当前选中值，和 mode 属性相关",
'onItemChange': "类型: function(key, d)\n默认值: 无\n说明: 节点点击触发事件",
'renderItem': "类型: string | function(d)\n默认值: 必填\n说明: 为 string 时，返回 d\\[string]<br />为 function 时，返回函数结果",
'renderResult': "类型: string | function(d, result)\n默认值: renderItem\n说明: 选中后在结果中显示的内容，默认和 renderItem 相同。返回 null 则不展示，result 为当前选中的所有值。",
'value': "类型: array\\[key]\n默认值: 无\n说明: 选中的 key （受控）",
